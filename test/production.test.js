/** 生产进度(主厂/动态加工点) + 验货问题 的后端测试（权限已统一：任意登录用户可读写，仅删加工点仍限管理员） */
...
(async () => {
  ...
  ok((await call("POST", `/orders/${o1.id}/inspections`, fT, { problems: ["下厂员想写"] })).status === 200, "权限统一后，下厂员也能创建「发现问题」");
  ...
  ok((await call("PATCH", `/orders/${o1.id}/inspections/${batch.id}/items/${itemId}`, fT, { problem: "下厂员想改" })).status === 200, "权限统一后，下厂员也能改「发现问题」");
  ok((await call("PATCH", `/orders/${o1.id}/inspections/${batch.id}/items/${itemId}`, fT, { fix: "整改好了" })).status === 200, "本单下厂员可填「整改情况」");
  ok((await call("PATCH", `/orders/${o1.id}/inspections/${batch.id}/items/${itemId}`, sT, { fix: "业务员想写fix" })).status === 200, "权限统一后，业务员也能填「整改情况」");
  ok((await call("PATCH", `/orders/${o1.id}/inspections/${batch.id}/items/${itemId}`, f2T, { fix: "无关下厂员" })).status === 200, "权限统一后，非本单下厂员也能填「整改情况」");

  // ---- 补充说明：任意登录用户都能加，累积不覆盖 ----
  ...
  ok((await call("POST", `/orders/${o1.id}/inspections/${batch.id}/items/${itemId}/notes`, f2T, { text: "补充说明3" })).status === 200, "权限统一后，无关下厂员也能加补充说明");
  ...
  ok(finalItem.notes.length === 3, "补充说明累计3条，互不覆盖");
  ok(finalItem.problem === "下厂员想改" && finalItem.fix === "无关下厂员", "problem/fix 各自的修改都正确保存(以最后一次写入为准)");
  ...
  ok((await call("PATCH", `/orders/${noFollowerOrder.j.id}/inspections/${nfCp.j.inspections[0].id}/items/${nfItemId}`, fT, { fix: "下厂员想填" })).status === 200,
    "没指定负责人时，权限统一后随便哪个下厂员都能填整改");
